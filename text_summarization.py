...
os.environ['OPENAI_API_KEY'] = 'open_ai_keya'
...
class ChatModelAPI:
    ...
    def __init__(self):
        self.chat_model = ChatOpenAI(temperature=0, openai_api_key=key, max_tokens=1000)
...
class DocumentQA(OpenAIObject):
    ...
    def run_engine(self,query):
        ...
        try:
            docsearch = FAISS.from_documents(self.create_batch() , self.get_embeddings())
...
class ContextMapping(ChatModelAPI):
    ...
    def generating_prompt_structure(self):
        ...
        try:
            ...
            return ChatPromptTemplate(
                ...
                partial_variables={"format_instructions": self.parsing_output_structure()}
            )
        except Exception as e:
            return e
    ...
    def get_context(self,userinput):
        ...
        try:
            _input = self.generating_prompt_structure().format_prompt(user_industries=userinput, standardized_industries=", ".join(self.df['Industry'].values))
